...
function onCharacterMessageReceived(mesId) {
    try {
        ...
        if (sdState.awaitingDiaryCapture && sdState.lastDiaryCapturedMesId !== idx) {
            ...
            if (parsed) {
                recordDiaryEntry(sdState, parsed);
...
<!--SD_DIARY_START
BODY: (the diary entry itself, first person, 3-6 sentences; a blank line between BODY's own paragraphs is fine)
...
function renderLastFiredRatingBlock(state) {
    const last = state.lastFiredEvent;
    if (!last) {
        return `<div class="sd-help">ยังไม่มีเหตุการณ์ล่าสุดให้ดูตอนนี้</div>`;
    }
    const label = `${HISTORY_KIND_LABELS[last.kind] || last.kind}: "${escapeHtml(truncateText(last.summary, 60))}"`;
    if (last.kind !== "fixed") {
        return `<div class="sd-help">เหตุการณ์ล่าสุด — ${label}<br/>ให้คะแนนได้เฉพาะเหตุการณ์ที่มาจากรายการที่คุณเขียนเอง (Fixed Pool) เท่านั้นครับ</div>`;
    }
    return `
...
function renderManualPickList(cfg) {
    if (!cfg.fixedPool.length) {
        return `<div class="sd-help">(ยังไม่มีเหตุการณ์ที่เขียนไว้ให้เลือกเลย ลองเพิ่มในช่อง Fixed Pool ด้านล่างก่อน)</div>`;
    }
    return cfg.fixedPool
        .map(
            (e, i) => `
...
function buildPopupHtml(cfg, state) {
    return `
    <div class="sd-popup">
        <div class="sd-stepper" id="sd-stepper">
            <div class="sd-stepper-header">
                <button type="button" class="sd-stepper-arrow sd-stepper-prev" id="sd-step-prev" aria-label="ย้อนกลับ" disabled>‹</button>
                <div class="sd-stepper-title-wrap">
                    <div class="sd-stepper-title" id="sd-step-title">⚙️ ทั่วไป</div>
                    <div class="sd-stepper-desc" id="sd-step-desc">เปิด-ปิดระบบ เลือกวิธีเล่าเหตุการณ์ และเสียงแจ้งเตือน</div>
                </div>
                <button type="button" class="sd-stepper-arrow sd-stepper-next" id="sd-step-next" aria-label="ถัดไป">›</button>
            </div>

            <div class="sd-stepper-track-viewport" id="sd-stepper-viewport">
                <div class="sd-stepper-track" id="sd-stepper-track">
        <div class="sd-step-panel" data-tab-panel="general">
            <div class="sd-row">
                <label><input type="checkbox" id="sd-enabled" ${cfg.enabled ? "checked" : ""}/> เปิดใช้งานกับแชทนี้</label>
            </div>
            <div class="sd-help">ปิดไว้ถ้าอยากพักระบบทั้งหมดชั่วคราวสำหรับแชทนี้ การตั้งค่าที่ทำไว้จะยังอยู่ครบ พร้อมเปิดกลับมาใช้ได้ทันที</div>

            <div class="sd-row">
                <label>รูปแบบการนำเสนอ</label>
                <select id="sd-presentation">
                    <option value="subtle" ${cfg.presentation === "subtle" ? "selected" : ""}>แบบแนบเนียน (บรรยายแทรกเข้าเนื้อเรื่องกลมกลืน ไม่ใช่ตัวละครพูดเอง)</option>
                    <option value="narrator" ${cfg.presentation === "narrator" ? "selected" : ""}>ผู้บรรยาย/GM แทรก (แยกชัดเจนจากเนื้อเรื่อง)</option>
                    <option value="both" ${cfg.presentation === "both" ? "selected" : ""}>ให้โมเดลเลือกเอง</option>
                </select>
            </div>
            <div class="sd-help">เลือกว่าจะให้เหตุการณ์ปรากฏในเรื่องแบบไหน — <b>แนบเนียน</b>: กลืนเข้าไปเป็นส่วนหนึ่งของคำบรรยายเหมือนเป็นเนื้อเรื่องปกติ ไม่มีใครมาบอกตรงๆ ว่ามีเหตุการณ์เกิดขึ้น | <b>ผู้บรรยาย/GM</b>: ขึ้นเป็นข้อความแยกที่เห็นชัดว่าไม่ใช่ส่วนของเนื้อเรื่อง | หรือปล่อยให้โมเดลเลือกเองตามความเหมาะสมของฉากนั้นๆ</div>
            ...
            <div class="sd-help">กดแล้วข้อความถัดไปจะมีเหตุการณ์เกิดขึ้นแน่นอน ไม่ต้องรอสุ่มหรือรอมิเตอร์สะสม (ยังใช้แหล่งเหตุการณ์และรูปแบบเดิมที่ตั้งไว้อยู่) มีผลทันที ไม่ต้องกด "บันทึก" แต่ถ้ามีเหตุการณ์ต่อเนื่องนัดไว้อยู่ก่อนแล้ว อันนั้นจะยังมาก่อนตามคิวเดิม</div>
            ...
            <div class="sd-help">เสียงสั้นๆ ที่จะเล่นพร้อมเอฟเฟกต์ด้านบนทุกครั้งที่มีเหตุการณ์เกิด กด "ลองฟังเสียง" เพื่อฟังตัวอย่างได้เลยตอนนี้ ไม่ต้องกด "บันทึก" ก่อน</div>
            ...
            <div class="sd-help">รีเซ็ตทุกหมวด (ทั่วไป/เหตุการณ์/ไดอารี่/ความสัมพันธ์) กลับเป็นค่าเริ่มต้น <b>เฉพาะแชทนี้เท่านั้น</b> แชทอื่นไม่กระทบ ไม่ลบ Fixed Pool ที่เขียนไว้ แท็กที่เลือก ประวัติ สถิติ ไดอารี่ หรือค่าความสัมพันธ์ — เห็นผลในหน้าต่างนี้ทันที แต่ต้องกด "บันทึก" ด้านล่างสุดก่อน การรีเซ็ตถึงจะคงอยู่ถาวร</div>
        </div>

        <div class="sd-step-panel" data-tab-panel="events">
            <div class="sd-subsection">
                ...
                <div class="sd-help">ตอนนี้แชทนี้อยู่ที่ <b id="sd-tension-value-text">${state.tension}</b> / 100 — ยิ่งค่าสูง เหตุการณ์ยิ่งมีโอกาสเกิดง่ายขึ้น พอเหตุการณ์เกิดแล้วค่านี้จะกลับไปเริ่มที่ 0 ใหม่ทันที</div>
                ...
                <div class="sd-help">ปิดไว้ถ้าไม่อยากให้ความนิ่งของเรื่องมีผลอะไร — โอกาสเกิดเหตุการณ์จะใช้แค่ค่า % คงที่ด้านล่างเท่านั้น ไม่ขยับขึ้นเองไม่ว่าเรื่องจะนิ่งแค่ไหน</div>
                ...
                <div class="sd-help">คงที่ = ค่าขึ้นเท่าเดิมทุกข้อความ คาดเดาจังหวะได้ง่าย<br/>ช่วงสุ่ม = ค่าขึ้นไม่เท่ากันแต่ละครั้ง ทำให้จับจังหวะเหตุการณ์ยากขึ้น เซอร์ไพรส์กว่า</div>

                <div class="sd-row">
                    <label>ค่าคงที่ที่เพิ่มต่อข้อความ</label>
                    <input type="number" id="sd-tension-fixed" value="${cfg.tensionGainFixed}" min="1" max="100"/>
                </div>
                <div class="sd-help">ใช้ตอนเลือกโหมด "คงที่" ด้านบน — ทุกข้อความค่านี้จะบวกเพิ่มเท่านี้เสมอ</div>
                ...
                <div class="sd-help">ใช้ตอนเลือกโหมด "ช่วงสุ่ม" ด้านบน — แต่ละข้อความจะสุ่มบวกค่าในช่วงที่กำหนดนี้</div>
                ...
                <div class="sd-help">พอมิเตอร์เต็ม 100 โอกาสเกิดเหตุการณ์ด้านล่างจะถูกบวกเพิ่มเท่านี้ทันที — ยิ่งเรื่องนิ่งนาน โอกาสเกิดเหตุการณ์ก็จะยิ่งสูงขึ้นเรื่อยๆ ไม่ปล่อยให้เนือยเกินไป</div>

                <div class="sd-row">
                    <label>ค่าที่บังคับให้เกิดเหตุการณ์ทันที</label>
                    <input type="number" id="sd-tension-cap" value="${cfg.tensionForceCap}" min="1" max="100"/>
                </div>
                <div class="sd-help">พอมิเตอร์แตะค่านี้ เหตุการณ์จะเกิดขึ้นในข้อความถัดไปแน่นอน ไม่ว่าจะสุ่มได้ผลอะไรก็ตาม ตั้งเป็น 100 ถ้าอยากให้บังคับเกิดก็ต่อเมื่อมิเตอร์เต็มจริงๆ เท่านั้น</div>
            </div>

            <div class="sd-subsection" style="margin-top:14px;">
                ...
                <div class="sd-help">เปิดไว้แล้วเหตุการณ์จะเกิดขึ้นทุกๆ N ข้อความแน่นอน เหมาะกับคนที่อยากให้จังหวะเรื่องสม่ำเสมอ<br/>ถ้ารู้สึกว่าเรื่องวนซ้ำหรือนิ่งไป ลองลดค่า N ลง (เช่น 4-6) เหตุการณ์จะโผล่มาถี่ขึ้นโดยไม่ต้องรอมิเตอร์สะสม</div>
                ...
                <div class="sd-help">โอกาสเริ่มต้นที่เหตุการณ์จะเกิดในแต่ละข้อความ ถ้าเปิดมิเตอร์ความตึงเครียดไว้ด้วย ค่านี้จะค่อยๆ ถูกบวกเพิ่มเองตามความนิ่งของเรื่อง</div>

                <div class="sd-row" style="margin-top:10px;">
                    <label>ความรุนแรงของเหตุการณ์</label>
                    <select id="sd-event-intensity">
                        <option value="mild" ${cfg.eventIntensity === "mild" ? "selected" : ""}>นุ่มนวล — แค่จุดสีสันเล็กๆ</option>
                        <option value="medium" ${cfg.eventIntensity === "medium" ? "selected" : ""}>ปานกลาง — เปลี่ยนแปลงชัดเจน</option>
                        <option value="disruptive" ${cfg.eventIntensity === "disruptive" ? "selected" : ""}>พลิกสถานการณ์แรง — ทำลายลูปเดิม</option>
                    </select>
                </div>
                <div class="sd-help">กำหนดว่าเหตุการณ์ที่เกิดขึ้นจะ "แรง" แค่ไหน<br/>เลือก "พลิกสถานการณ์แรง" ถ้าอยากให้เหตุการณ์เปลี่ยนสถานที่/สถานการณ์/เดิมพันแบบจริงจังชัดเจน ไม่ใช่แค่จุดสีสันเล็กๆ ที่กลืนหายไปกับฉากเดิม เหมาะกับตอนที่รู้สึกว่าเรื่องวนซ้ำหรือนิ่งเกินไป</div>
            </div>
            ...
            <div class="sd-subsection" style="margin-top:14px;">
                ...
                <div class="sd-help">พอมีเหตุการณ์เกิดขึ้นครั้งหนึ่ง ระบบอาจนัดให้เธรดเดิมถูกหยิบกลับมาสานต่อ/ทวีความเข้มข้นขึ้นอีกในอีกไม่กี่ข้อความถัดไป แทนที่จะสุ่มเหตุการณ์ใหม่ที่ไม่เกี่ยวกันไปเรื่อยๆ — โมเดลจะย้อนอ่านบทสนทนาเองว่าเรื่องไปถึงไหนแล้วก่อนสานต่อให้ลื่นไหล</div>
                ...
                <div class="sd-help">ทุกครั้งที่มีเหตุการณ์เกิด (ทั้งเหตุการณ์ใหม่หรือเหตุการณ์ต่อเนื่องเอง) จะมีโอกาสเท่านี้ที่จะถูกนัดให้สานต่ออีกครั้ง</div>
                ...
                <div class="sd-help">หลังเหตุการณ์ต้นทางเกิดขึ้น ระบบจะสุ่มว่าต้องรออีกกี่ข้อความในช่วงนี้ ก่อนเหตุการณ์ต่อเนื่องจะแทรกเข้ามา</div>
                ...
                <div class="sd-help">กันไม่ให้เธรดเดียวถูกสานต่อไม่จบไม่สิ้น เช่นตั้งเป็น 2 = เรื่องเดิมสานต่อได้สูงสุด 2 ครั้ง ก่อนจะกลับไปสุ่มเหตุการณ์ใหม่ตามปกติ ตั้งเป็น 0 ถ้าไม่อยากให้สานต่อเป็นสาย (นัดได้แค่ครั้งเดียวจบ)</div>
            ...
            <div class="sd-subsection" style="margin-top:14px;">
                ...
                <div class="sd-help">แทนที่จะแทรกเหตุการณ์เข้าไปในฉากปัจจุบัน โมเดลจะข้ามเวลาไปข้างหน้าแทน สรุปช่วงที่ข้ามสั้นๆ แล้วเริ่มฉากใหม่เลย เป็นคนละระบบกับเหตุการณ์ปกติด้านบน จะไม่เกิดซ้อนกันในข้อความเดียวกัน</div>
                ...
                <div class="sd-help">กดแล้วมีผลทันที ไม่ต้องกด "บันทึก" — ถ้ามีเหตุการณ์ต่อเนื่องนัดไว้อยู่ก่อน จะข้ามไปก่อนเหมือนปุ่ม "สั่งเหตุการณ์ทันที" ด้านบน</div>
                ...
                <div class="sd-help">ใช้ได้เมื่อเปิด "ฟีเจอร์ข้ามเวลา" ไว้ด้านบน เป็นโอกาสแยกต่างหากจากเหตุการณ์ปกติ ถ้าข้ามเวลาเกิดในข้อความไหน เหตุการณ์ปกติจะไม่เกิดซ้อนในข้อความนั้น ตั้งเป็น 0 ถ้าอยากให้ข้ามเวลาได้เฉพาะตอนกดปุ่มเองเท่านั้น</div>
                ...
                <div class="sd-help">"ให้โมเดลเลือกเอง" จะไม่กำหนดตัวเลขตายตัวไว้ล่วงหน้า แต่ให้โมเดลเลือกระยะเวลาที่เหมาะกับฉากตอนนั้นเอง</div>
                ...
                <div class="sd-help">ทุกครั้งที่ข้ามเวลา ระบบจะสุ่มจำนวนในช่วงนี้ (หน่วยเดียวกันทั้งคู่) แล้วบอกโมเดลตรงๆ ว่าข้ามไปเท่าไร</div>
                ...
                <div class="sd-help">แนะนำให้เปิดไว้ — กันไม่ให้เหตุการณ์ปกติหรือข้ามเวลาอีกครั้งเกิดซ้อนทันทีในข้อความถัดไป และจะยกเลิกเหตุการณ์ต่อเนื่องที่นัดไว้ก่อนหน้าด้วย เพราะมันอ้างอิงถึงช่วงเวลาที่ผ่านไปแล้ว</div>
            </div>
            ...
            <div class="sd-subsection" style="margin-top:14px;">
                ...
                <div class="sd-help">Fixed Pool = สุ่มจากรายการเหตุการณ์ที่คุณเขียนไว้เอง คุมทิศทางเรื่องได้แม่นยำ<br/>AI = ให้โมเดลแต่งเหตุการณ์สดใหม่จากธีมด้านล่างเอง ไม่ต้องเตรียมล่วงหน้า และไม่ซ้ำเดิม</div>
                ...
                <div class="sd-help">เลือกได้หลายแท็ก — โหมด AI: โมเดลจะพยายามแต่งเหตุการณ์ให้เข้ากับแท็กที่เลือกไว้<br/>โหมด Fixed Pool: ถ้าเหตุการณ์ในรายการติด #tag ไว้ (ดูด้านล่าง) ระบบจะสุ่มจากเหตุการณ์ที่ตรงแท็กก่อน ถ้าไม่มีตรงเลยถึงจะสุ่มจากทั้งหมด</div>
                ...
                <div class="sd-help">ส่วนก่อน "::" แรก คือหัวข้อสั้นๆ ที่ใช้จำไว้กันไม่ให้ออกซ้ำ ส่วนถัดมาคือรายละเอียดเต็มที่จะส่งให้โมเดลตอนเหตุการณ์นี้ถูกเลือก<br/>ใส่ <code>#tag1,tag2</code> ท้ายบรรทัดได้ (ไม่ใส่ก็ได้) เพื่อให้กรองตามแท็กธีมด้านบนได้<br/>ใส่ <code>xN</code> ท้ายบรรทัดได้ (ค่าเริ่มต้น x1) เพื่อเพิ่มโอกาสถูกสุ่ม เช่น <code>x3</code> = มีโอกาสออกมากกว่าปกติ 3 เท่า ยิ่งค่าสูงยิ่งออกบ่อย ตัวอย่าง: <code>ปีศาจบุกหมู่บ้าน :: กลุ่มปีศาจโจมตีหมู่บ้านกลางดึก :: #Horror,Action :: x3</code></div>
                ...
                <div class="sd-help">กด "ยิงอันนี้" เพื่อให้เหตุการณ์นั้นเกิดขึ้นในข้อความถัดไปแน่นอน ข้ามการสุ่มไปเลย มีผลทันที ไม่ต้องกด "บันทึก" เหมาะตอนอยากคุมทิศทางเรื่องเอง — รายการนี้แสดงเฉพาะที่บันทึกไว้แล้วเท่านั้น ถ้าเพิ่งแก้กล่องข้อความด้านบน ให้กด "บันทึก" แล้วเปิดหน้าต่างนี้ใหม่ก่อน รายการถึงจะอัปเดต</div>

                <div class="sd-row" style="margin-top:10px;">
                    <button type="button" class="menu_button" id="sd-blind-pick-btn">สุ่มจากกล่องลึกลับ (ไม่รู้ล่วงหน้า)</button>
                </div>
                <div class="sd-help">ต่างจาก "ยิงอันนี้" ตรงที่คุณจะไม่รู้ล่วงหน้าว่าได้อะไร — ระบบสุ่มจากพูลทันที (ให้น้ำหนักและกรองแท็กเหมือนสุ่มปกติ) แล้วเก็บเป็นความลับไว้ก่อน จะรู้ผลตอนเอฟเฟกต์เผยขึ้นในข้อความถัดไป (หรือตอนอ่านข้อความเลย ถ้าปิดเอฟเฟกต์ไว้)</div>
            </div>
            ...
            <div class="sd-subsection" style="margin-top:14px;">
                ...
                <div class="sd-help">ดูล่วงหน้าได้เลยว่าคำสั่งจริงที่จะส่งให้โมเดลหน้าตาเป็นแบบไหน โดยใช้ค่าที่กำลังแก้อยู่ตอนนี้ (ยังไม่ต้องกด "บันทึก") แค่ดูเฉยๆ ไม่กระทบแชทจริงหรือประวัติเหตุการณ์แต่อย่างใด</div>
        ...
        <div class="sd-step-panel" data-tab-panel="diary">
            <div class="sd-subsection">
                ...
                <div class="sd-help">พอถึงรอบ ตัวละครจะแต่งไดอารี่สดขึ้นมาเอง เนื้อหา ป.ล. และอารมณ์ล้วนเขียนโดยโมเดลเอง ส่วนวันที่ สภาพอากาศ และเวลาที่เขียน ระบบจะสุ่มให้ท้ายสุด — มันจะมาพร้อมกับคำตอบปกติเลย ไม่ต้องรอสร้างข้อความเพิ่มแยกต่างหาก และเกิดร่วมกับเหตุการณ์ในข้อความเดียวกันได้</div>
                ...
                <div class="sd-help">เปิดพร้อมกันทั้งสองแบบได้ — จะปลดล็อกทันทีที่ถึงเงื่อนไขไหนก่อน นับจากข้อความของตัวละคร ไม่ใช่ของคุณ และเกิดร่วมกับเหตุการณ์หรือการข้ามเวลาในข้อความเดียวกันได้เลย ไม่ชนกัน</div>
                ...
                <div class="sd-help">กด "เขียนตอนนี้" แล้วตัวละครจะเขียนไดอารี่ในคำตอบถัดไปเลย มีผลทันที ไม่ต้องกด "บันทึก"</div>
            </div>

            <div class="sd-subsection" style="margin-top:14px;">
                <h4>กิมมิคในเนื้อหา</h4>
                <div class="sd-help">
                    ตอนตัวละครเขียนไดอารี่ อาจมีลูกเล่นพวกนี้แทรกอยู่ในเนื้อหา ไม่ต้องตั้งค่าอะไร แค่รู้ไว้เผื่ออยากเข้าใจว่าสีไหนหมายถึงอะไรตอนอ่าน: <code>~~ข้อความ~~</code> = ขีดฆ่าสีจาง (ความคิดที่กลั้นไว้ไม่พูดออกมา), <code>==ข้อความ==</code> = ไฮไลท์เหลือง (ความจริงที่ยอมรับ), <code>__ข้อความ__</code> = ขีดเส้นใต้สีน้ำเงิน (สิ่งที่ตั้งใจไว้แน่วแน่), <code>\`\`ข้อความ\`\`</code> (แบ็กทิกคู่) = ขีดเส้นใต้จุดสีชมพู (คำที่พูดออกไปทั้งที่ใจไม่ตรงกัน)
                </div>
            </div>
        ...
        <div class="sd-step-panel" data-tab-panel="stats">
            <div class="sd-subsection">
                <h4>สถิติ</h4>
            ...
            <div class="sd-subsection" style="margin-top:14px;">
                ...
                <div class="sd-help">ระบบจะจำเหตุการณ์ล่าสุดไว้เท่านี้รายการ แล้วบอกโมเดลให้เลี่ยงไม่ทำเหตุการณ์ซ้ำกับที่จำไว้<br/>ยิ่งจำเยอะยิ่งกันเหตุการณ์ซ้ำได้ดีขึ้น แต่จะกินโทเค็นเพิ่มขึ้นเล็กน้อย</div>
                ...
                <div class="sd-help">เหตุการณ์ทั้งหมดที่เคยเกิดในแชทนี้ พร้อมเวลาและประเภท (Fixed Pool / AI สร้าง / ต่อเนื่อง) — ไว้ดูย้อนหลังอย่างเดียว ไม่ถูกส่งเข้าพรอมต์ (มีแค่รายการ "ล่าสุด" ด้านบนเท่านั้นที่ใช้กันเหตุการณ์ซ้ำ)</div>
                <div class="sd-full-history" id="sd-full-history">${renderFullHistory(state)}</div>
            </div>
        </div>

        <div class="sd-step-panel" data-tab-panel="io">
            ...
            <div class="sd-help" id="sd-io-status">เก็บชุดการตั้งค่าของแชทนี้เป็นไฟล์ไว้สำรอง หรือนำไปใช้กับแชท/ตัวละครอื่นได้เลย (ไม่รวมค่าความตึงเครียดหรือประวัติเหตุการณ์ปัจจุบัน เพราะเป็นสถานะเฉพาะของแชทนี้) นำเข้าไฟล์แล้วต้องกด "บันทึก" ด้านล่างอีกครั้ง การตั้งค่าถึงจะถูกใช้จริง</div>
        </div>
                </div>
            </div>

            <div class="sd-stepper-dots" id="sd-stepper-dots">
                <button type="button" class="sd-dot active" data-tab="general" aria-label="ทั่วไป"></button>
                <button type="button" class="sd-dot" data-tab="events" aria-label="เหตุการณ์"></button>
                <button type="button" class="sd-dot" data-tab="diary" aria-label="ไดอารี่">${state.diaryLog.some((e) => !e.read) ? `<span class="sd-dot-badge" id="sd-diary-dot-badge">${state.diaryLog.filter((e) => !e.read).length}</span>` : ""}</button>
                <button type="button" class="sd-dot" data-tab="stats" aria-label="สถิติ/ประวัติ"></button>
                <button type="button" class="sd-dot" data-tab="io" aria-label="นำเข้า/ส่งออก"></button>
            </div>
        </div>
...
function wireImportExport(draft) {
    ...
    const fileHandler = async (e) => {
        ...
        try {
            ...
            notify("นำเข้าไฟล์ไม่สำเร็จ — ตรวจสอบว่าเป็นไฟล์ .json ที่ส่งออกจาก Story Director", true);
        } finally {
            e.target.value = "";
        }
    };
...
// Pure UI state — a Stepper/carousel that pages through the settings
// sections one at a time (arrows + dots). Doesn't touch cfg, draft, or any
// saved data at all, aside from marking diary entries read on arrival.
const SD_TAB_ORDER = ["general", "events", "diary", "stats", "io"];
const SD_TAB_META = {
    general: { title: "⚙️ ทั่วไป", desc: "เปิด-ปิดระบบ เลือกวิธีเล่าเหตุการณ์ และเสียงแจ้งเตือน" },
    events: { title: "🎲 เหตุการณ์", desc: "ตั้งจังหวะ ความแรง และที่มาของเหตุการณ์ในเรื่อง" },
    diary: { title: "📔 ไดอารี่", desc: "สมุดบันทึกส่วนตัวของตัวละคร เขียนขึ้นเองเป็นระยะ" },
    stats: { title: "📊 สถิติ/ประวัติ", desc: "ย้อนดูเหตุการณ์ทั้งหมดที่เคยเกิดในแชทนี้" },
    io: { title: "📁 นำเข้า/ส่งออก", desc: "บันทึกการตั้งค่าเป็นไฟล์ หรือโหลดจากไฟล์ที่เคยเซฟไว้" },
};

// Drag-to-swipe needs a bit of state that survives across touch events but
// shouldn't leak between popup opens — kept local to wireNav() below.
function wireNav() {
    let currentIndex = 0; // "general" — matches the markup baked in buildPopupHtml
    let dragStartX = null;
    let dragDeltaPx = 0;
    let trackWidthPx = 0;

    const getTrack = () => document.getElementById("sd-stepper-track");
    ...
    const markDiaryRead = () => {
        ...
        saveChatState();
        const badge = document.getElementById("sd-diary-dot-badge");
    ...
    const goToIndex = (index) => {
        const clamped = Math.max(0, Math.min(SD_TAB_ORDER.length - 1, index));
        currentIndex = clamped;
        const tab = SD_TAB_ORDER[clamped];
        const meta = SD_TAB_META[tab] || {};

        const track = getTrack();
        if (track) {
            track.style.setProperty("--sd-drag", "0px");
            track.style.setProperty("--sd-step", String(clamped));
        }

        const titleEl = document.getElementById("sd-step-title");
        if (titleEl) titleEl.textContent = meta.title || "";
        const descEl = document.getElementById("sd-step-desc");
        if (descEl) descEl.textContent = meta.desc || "";

        const prevBtn = document.getElementById("sd-step-prev");
        const nextBtn = document.getElementById("sd-step-next");
        if (prevBtn) prevBtn.disabled = clamped === 0;
        if (nextBtn) nextBtn.disabled = clamped === SD_TAB_ORDER.length - 1;

        document.querySelectorAll(".sd-dot").forEach((dot) => {
            dot.classList.toggle("active", dot.dataset.tab === tab);
        });

        // A small entrance flourish on the panel that just became current —
        // matches the "revealed one step at a time" feel of the carousel.
        const panel = document.querySelector(`.sd-step-panel[data-tab-panel="${tab}"]`);
        if (panel) {
            panel.classList.remove("sd-step-enter");
            void panel.offsetWidth;
            panel.classList.add("sd-step-enter");
        }

        const popupEl = document.querySelector(".sd-popup");
        if (popupEl) popupEl.scrollTop = 0;

        if (tab === "diary") markDiaryRead();
    };

    const handleClick = (e) => {
        const prev = e.target.closest ? e.target.closest("#sd-step-prev") : null;
        if (prev) { goToIndex(currentIndex - 1); return; }

        const next = e.target.closest ? e.target.closest("#sd-step-next") : null;
        if (next) { goToIndex(currentIndex + 1); return; }

        const dot = e.target.closest ? e.target.closest(".sd-dot") : null;
        if (dot) {
            const idx = SD_TAB_ORDER.indexOf(dot.dataset.tab);
            if (idx !== -1) goToIndex(idx);
        }
    };

    const handleKeydown = (e) => {
        // Only steer the carousel with arrow keys when focus isn't inside a
        // text field/textarea/select, so typing numbers or text still works.
        const tag = e.target && e.target.tagName;
        if (tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT") return;
        if (!document.getElementById("sd-stepper")) return;
        if (e.key === "ArrowLeft") { goToIndex(currentIndex - 1); }
        else if (e.key === "ArrowRight") { goToIndex(currentIndex + 1); }
    };

    // Touch-swipe support so the carousel can be dragged like a real story
    // page, not just clicked through with the arrows/dots.
    const handleTouchStart = (e) => {
        const viewport = e.target.closest ? e.target.closest("#sd-stepper-viewport") : null;
        if (!viewport || e.touches.length !== 1) return;
        dragStartX = e.touches[0].clientX;
        dragDeltaPx = 0;
        trackWidthPx = viewport.getBoundingClientRect().width || 1;
        const track = getTrack();
        if (track) track.classList.add("sd-dragging");
    };

    const handleTouchMove = (e) => {
        if (dragStartX === null || e.touches.length !== 1) return;
        dragDeltaPx = e.touches[0].clientX - dragStartX;
        // Resist dragging further past the first/last step.
        if ((currentIndex === 0 && dragDeltaPx > 0) || (currentIndex === SD_TAB_ORDER.length - 1 && dragDeltaPx < 0)) {
            dragDeltaPx *= 0.35;
        }
        const track = getTrack();
        if (track) track.style.setProperty("--sd-drag", `${dragDeltaPx}px`);
    };

    const handleTouchEnd = () => {
        if (dragStartX === null) return;
        const track = getTrack();
        if (track) track.classList.remove("sd-dragging");
        const threshold = Math.max(40, trackWidthPx * 0.18);
        if (dragDeltaPx <= -threshold) goToIndex(currentIndex + 1);
        else if (dragDeltaPx >= threshold) goToIndex(currentIndex - 1);
        else goToIndex(currentIndex); // snap back to where we already were
        dragStartX = null;
        dragDeltaPx = 0;
    };
    ...
    // Note: this function runs *before* callGenericPopup() actually mounts
    // `html` into the DOM, so the initial title/desc/dot/disabled-arrow
    // state can't be set from here — it's baked directly into the markup
    // in buildPopupHtml() instead (index 0 / "general") and only needs to
    // stay in sync with SD_TAB_ORDER[0] and SD_TAB_META.general above.
