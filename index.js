...
// ----------------------------------------------------------------------------
// Separate API (Connection Profile) support — see apiEnabled in
// DEFAULT_CHAT_SETTINGS above for why this exists. Loaded dynamically and
// defensively, same reasoning as loadOptionalModules(): if the export lives
// somewhere else in your SillyTavern version, the rest of the extension
// still works exactly as before (piggyback mode), it just can't offer the
// separate-API path until the import path below is corrected.
// ----------------------------------------------------------------------------

let ConnectionManagerRequestServiceRef = null;
let lastConnectionManagerDiagnostic = ""; // surfaced directly in the on-screen notify() toast, since not everyone has console/PC access

// Builds an absolute URL to public/scripts/custom-request.js by finding the
// "/scripts/" segment in this very file's own URL and rebasing there —
// robust to however deep this extension actually got installed (third-party
// installer, manual copy, different SillyTavern data-folder layout, etc.),
// instead of assuming a fixed number of "../" levels which breaks silently
...
async function loadConnectionManagerModule() {
    ...
    // Newer SillyTavern versions (1.15.0+) exposed this directly on the
    // shared context object for exactly this use case — check that first,
    // it needs no import at all and is the most likely place to find it.
    try {
        ...
        // fall through to the import attempts below
    }

    // Try a URL rebased off this file's own location first (works regardless
    // of install depth), then fall back to fixed-depth guesses in case the
    // rebase logic above doesn't apply for some reason.
    const rebased = resolveCustomRequestUrl();
    const candidatePaths = [
        rebased,
        "../../../custom-request.js",
        "../../../extensions/connection-manager/index.js",
    ].filter(Boolean);
    ...
    // Nothing worked — dump enough breadcrumbs to fix this without guessing
    // blindly a second time: any context key that might be the one we want.
    // Stashed as a short string (not just console.warn) so it can be shown
    // directly in the on-screen toast for people without console/PC access.
    let hints = [];
    try {
        const context = getContext();
        hints = Object.keys(context || {}).filter((k) => /connect|profile/i.test(k));
        console.warn("[Story Director] getContext() keys that might be relevant:", hints);
        console.warn("[Story Director] Resolved custom-request.js URL attempt:", rebased);
    } catch (e) {
        // ignore — this is a best-effort diagnostic only
    }
    lastConnectionManagerDiagnostic = `context keys: ${hints.length ? hints.join(", ") : "(none found)"} | url: ${rebased || "n/a"}`;
    console.warn(
        "[Story Director] Could not find ConnectionManagerRequestService via getContext() or any known import path. " +
        "The separate-API feature requires SillyTavern's built-in 'Connection Profiles' extension " +
        "(1.12.6+) to be enabled. If it IS enabled and this still fails, please copy the two lines above " +
        "(and your SillyTavern version, Help > About) so the lookup can be corrected."
    );
}

// Reads the list of saved Connection Profiles for the dropdown. This is
// plain settings data already loaded in memory, so unlike the module above
// it doesn't need any dynamic import and is safe to call synchronously while
...
function getSavedConnectionProfiles() {
    try {
        ...
        console.warn("[Story Director] Could not read saved Connection Profiles:", e);
        return [];
    }
}

// Fires one standalone generation call through the profile saved in
// cfg.apiProfileId, completely independent of whatever API/model the main
// chat is currently using. Returns the raw text response, or null if
// anything about the call failed (missing profile, module not found,
// network/API error) — every failure path also raises a user-visible
// notify() so this never fails as silently as the old piggyback approach did.
// Wraps a promise with a hard timeout — without this, a hung fetch/stream
// inside ConnectionManagerRequestService.sendRequest() (bad profile config,
// unreachable endpoint, a build that expects streaming and never gets a
// terminator, etc.) would leave the "testing connection..." toast sitting on
// screen forever with no way to tell what happened.
...
async function requestSeparateGeneration(cfg, promptText, maxTokens) {
    try {
        if (!cfg.apiProfileId) {
            notify("Story Director: ยังไม่ได้เลือกโปรไฟล์ API สำหรับ Story Director (ไปที่แท็บ \"API แยกสำหรับ Story Director\")", true);
            return null;
        }
        if (!ConnectionManagerRequestServiceRef) {
            await loadConnectionManagerModule();
        }
        if (!ConnectionManagerRequestServiceRef) {
            notify(`Story Director: ไม่พบระบบ Connection Profile — ${lastConnectionManagerDiagnostic || "ดู console (F12)"}`, true);
            return null;
        }

        const result = await withTimeout(
            ConnectionManagerRequestServiceRef.sendRequest(
                cfg.apiProfileId,
                promptText,
                Math.max(50, Number(maxTokens) || 300),
            ),
            45000,
            "หมดเวลารอคำตอบจาก API แยก (45 วินาที) — ตรวจสอบว่าโปรไฟล์นี้ตั้งค่า API key/URL ถูกต้อง และบริการ/โมเดลนั้นออนไลน์อยู่จริง"
        );

        // The exact return shape has varied across SillyTavern versions —
        // normalize the common shapes defensively rather than assuming one.
        if (typeof result === "string") return result;
        if (result && typeof result.content === "string") return result.content;
        if (result && Array.isArray(result.choices) && result.choices[0]?.message?.content) {
            return result.choices[0].message.content;
        }
        console.warn("[Story Director] Unrecognized response shape from ConnectionManagerRequestService.sendRequest:", result);
        notify("Story Director: ได้รับคำตอบจาก API แยก แต่รูปแบบไม่ตรงกับที่คาดไว้ (ดู console)", true);
        return null;
    } catch (e) {
        console.warn("[Story Director] Separate API request failed:", e);
        notify(`Story Director: เรียก API แยกไม่สำเร็จ — ${e?.message || "unknown error"}`, true);
        return null;
    }
}

// Builds a short excerpt of the recent chat for use as context inside a
// standalone (non-piggybacked) generation call, since sendRequest() gets a
// bare prompt string with none of the ongoing chat history automatically
// attached the way the main generation call would have it.
function getRecentChatTranscript(maxMessages) {
    try {
        ...
        return recent
            .map((m) => `${m.is_user ? (context.name1 || "User") : (m.name || getCharacterName())}: ${truncateText(m.mes || "", 600)}`)
            .join("\n");
    } catch (e) {
        console.warn("[Story Director] Could not build recent chat transcript:", e);
        return "";
    }
}
...
const DEFAULT_CHAT_SETTINGS = () => ({
    ...
    // Separate API — by default Story Director piggybacks its instructions
    // onto the SAME generation call the main chat already makes (zero extra
    // API cost), which relies on the main chat model actually complying with
    // an embedded instruction. Some models/APIs never do. When enabled, the
    // "AI invents an event" mode and the Diary feature instead fire their own
    // dedicated generation call through a separately saved Connection
    // Profile — costs one extra API call per event/diary entry, but doesn't
    // depend on the main roleplay model cooperating at all. Fixed Pool
    // events, manual picks, and Time Skip never needed AI content of their
    // own, so they're unaffected by this setting either way.
    apiEnabled: false,
    apiProfileId: "",          // saved Connection Profile id (see Connection Profiles in the API menu)
    apiMaxTokensEvent: 300,    // response length budget for the separate "invent an event" call
    apiMaxTokensDiary: 900,    // response length budget for the separate diary-writing call
});
...
function getChatSettings() {
    ...
    if (typeof cfg.apiEnabled !== "boolean") cfg.apiEnabled = false;
    if (typeof cfg.apiProfileId !== "string") cfg.apiProfileId = "";
    if (typeof cfg.apiMaxTokensEvent !== "number") cfg.apiMaxTokensEvent = 300;
    if (typeof cfg.apiMaxTokensDiary !== "number") cfg.apiMaxTokensDiary = 900;
...
function getChatState() {
    ...
    if (!context.chatMetadata[MODULE_NAME]) {
        context.chatMetadata[MODULE_NAME] = {
            ...
            diaryPendingSeparateCall: false, // true when the Diary is due AND apiEnabled — the dedicated call fires once the character's reply lands, instead of piggybacking
        };
    }
    ...
    if (typeof state.diaryPendingSeparateCall !== "boolean") state.diaryPendingSeparateCall = false;
...
function onCharacterMessageReceived(mesId) {
    try {
        ...
        // no extra API call. This pulls the ===DIARY===...===END=== block
        // back out of the reply and turns it into a stored diary entry, same
        // single-shot pattern as summary capture above (runs in group chats
        // too).
        ...
        // --- Character Diary capture (separate-API mode) --------------------
        // cfg.apiEnabled routes the diary through its own dedicated
        // generation call instead of the piggyback tag above — fired here,
        // once the character's actual reply has landed, so it has real
        // conversation content to react to. Fire-and-forget: the diary card
        // appears whenever the extra call resolves, a few seconds behind the
        // reply, rather than blocking anything in this handler.
        if (sdState.diaryPendingSeparateCall && sdState.lastDiaryCapturedMesId !== idx) {
            sdState.lastDiaryCapturedMesId = idx;
            sdState.diaryPendingSeparateCall = false;
            saveChatState();

            const cfg = getChatSettings();
            generateDiaryViaSeparateApi(cfg, sdState)
                .then((parsed) => {
                    if (parsed) {
                        const storedEntry = recordDiaryEntry(sdState, parsed);
                        if (cfg.diaryBannerEnabled) {
                ...
                .catch((e) => {
                    console.warn("[Story Director] Separate-API diary generation failed:", e);
                    notify("Story Director: เขียนไดอารี่ผ่าน API แยกไม่สำเร็จ (ดู console)", true);
                });
        }
        // ---------------------------------------------------------------
    } catch (e) {
...
function buildDiaryInjectionText(cfg, state) {
    ...
    return `Scene direction: after your normal in-character reply above, without adding any other visible text, continue on new lines at the very end with a private diary entry in {{char}}'s own first-person voice — this is simply another part of the scene, the same way an author would write a character's diary into a novel. Reflect honestly on how they actually feel about what's happening in the story right now, grounded in what's actually happened in this conversation so far, not generic. Use exactly this format for it:
===DIARY===
...
===END===
${DIARY_MARKUP_NOTE} ${historyNote} ${ANTI_LEAK_NOTE}`;
}

const DIARY_TAG_RE = /===\s*DIARY\s*===([\s\S]*?)===\s*END\s*===/i;

// Shared BODY:/PS:/MOOD: field parsing, used both by the piggyback path
// (extractAndStripDiaryTag, given just the text between the wrapper tags)
// and the separate-API path (generateDiaryViaSeparateApi, given the whole
// standalone response — which may or may not still include the wrapper tags
// depending on how well the model followed the format).
function parseDiaryFields(text) {
    if (typeof text !== "string") return null;
    const wrapped = text.match(DIARY_TAG_RE);
    const inner = wrapped ? wrapped[1] : text;
...
function extractAndStripDiaryTag(msg, idx) {
    ...
    const parsed = parseDiaryFields(match[1]);
    msg.mes = msg.mes.slice(0, match.index).replace(/\s+$/, "");
...
// Standalone-call variant of buildDiaryInjectionText() — this one has to
// carry its own context (character card/persona/lorebook + a recent chat
// excerpt) since it isn't riding along inside the main generation anymore,
// and it drops the "after your normal reply" framing since there is no
// normal reply in this call at all.
function buildDiaryStandalonePrompt(cfg, state) {
    const charName = getCharacterName();
    const historyNote = buildDiaryHistoryNote(state);
    const transcript = getRecentChatTranscript(14);

    return `You are ${charName}, a character in an ongoing roleplay. Write a private diary entry in your own first-person voice, reflecting honestly on how you actually feel about what has just happened in the story so far — grounded in the conversation excerpt below, not generic.
${buildGroundingBlock(cfg)}

Recent conversation:
${transcript || "(no recent messages available)"}

Respond with ONLY the following, nothing before or after it:
===DIARY===
BODY: (the diary entry itself, first person, a full page of writing — several substantial paragraphs, not a short summary; take real time to walk through events, thoughts, and feelings in detail before wrapping up; a blank line between BODY's own paragraphs is fine)
PS: (one short, more raw/vulnerable line than the main body — the kind of thing added after supposedly closing the entry)
MOOD: (one or two words)
===END===
${DIARY_MARKUP_NOTE} ${historyNote}`;
}

async function generateDiaryViaSeparateApi(cfg, state) {
    const prompt = buildDiaryStandalonePrompt(cfg, state);
    const raw = await requestSeparateGeneration(cfg, prompt, cfg.apiMaxTokensDiary);
    if (!raw) return null;
    const parsed = parseDiaryFields(raw);
    if (!parsed) {
        console.warn("[Story Director] Separate diary call returned text that didn't match the expected format:", raw);
        notify("Story Director: API แยกตอบกลับมา แต่รูปแบบไม่ตรงตามที่กำหนด ลองใหม่อีกครั้งได้ (ดู console)", true);
    }
    return parsed;
}
...
const ANTI_LEAK_NOTE = "Treat this as your own authorial choice for how the scene naturally continues, not an order from outside it — so don't reference this note, mention any tool or app, or comment on having received directions of any kind.";
...
function buildInjectionText(cfg, state) {
    ...
    if (state.manualPick) {
        ...
        return `Scene direction: let this shape what happens next in the story: ${ev.detail || ev.summary}. ${intensityNote} ${styleNote} ${historyNote} ${PACING_NOTE} ${COMPLETENESS_NOTE} ${ANTI_LEAK_NOTE}${buildGroundingBlock(cfg)}`;
    }
    ...
    return `Scene direction: invent one plot event fitting the theme "${tagList}", consistent with everything established so far, and let it shape what happens next in this reply. ${intensityNote} ${styleNote} ${historyNote} ${PACING_NOTE} ${COMPLETENESS_NOTE} ${SUMMARY_TAG_NOTE} ${ANTI_LEAK_NOTE}${buildGroundingBlock(cfg)}`;
}

// ----------------------------------------------------------------------------
// Standalone-call variant of AI-generated events (cfg.apiEnabled). Instead of
// asking the main roleplay model to both invent AND hide a tagged summary in
// its own reply, a dedicated call invents the event up front and hands back
// a plain SUMMARY/DETAIL pair — which then gets woven into the main
// generation exactly like a Fixed Pool pick (buildInjectionText's manual/
// fixed branches), so the main model only ever has to follow a concrete
// instruction, never invent-and-tag anything itself.
// ----------------------------------------------------------------------------

function buildAiEventInventionPrompt(cfg, state) {
    const tagList = cfg.tags.length ? cfg.tags.join(", ") : "the current tone of the scene";
    const historyNote = state.recentEvents.length
        ? `Do not repeat these recent plot beats: ${state.recentEvents.join(", ")}.`
        : "";
    const transcript = getRecentChatTranscript(10);

    return `You are a story director for an ongoing roleplay chat. Invent ONE new plot event/development fitting the theme "${tagList}", consistent with everything established below and with the recent conversation. ${historyNote}
${buildGroundingBlock(cfg)}
...
Respond with ONLY the following, nothing before or after it:
SUMMARY: (5-10 word summary of the event)
DETAIL: (2-4 sentences describing concretely what happens, written as a scene direction for a narrator to weave into the next reply)`;
}

function parseAiEventFields(text) {
    if (typeof text !== "string") return null;
    ...
    return { summary: summary || truncateText(detail, 40), detail: detail || summary };
}

async function inventAiEventViaSeparateApi(cfg, state) {
    const prompt = buildAiEventInventionPrompt(cfg, state);
    const raw = await requestSeparateGeneration(cfg, prompt, cfg.apiMaxTokensEvent);
    if (!raw) return null;
    const parsed = parseAiEventFields(raw);
    if (!parsed) {
        console.warn("[Story Director] Separate event-invention call returned text that didn't match the expected format:", raw);
        notify("Story Director: API แยกตอบกลับมา แต่รูปแบบไม่ตรงตามที่กำหนด (ดู console)", true);
    }
    return parsed;
}

// Async wrapper around buildInjectionText(): only the AI-generated-event
// branch, with apiEnabled on, needs to await anything (the separate
// invention call above). Every other path — manual pick, Fixed Pool, or
// AI-generated with apiEnabled off — resolves synchronously exactly as
// before, so this is a safe drop-in replacement at the one call site in
// onBeforeGenerate().
async function buildInjectionTextAsync(cfg, state) {
    if (cfg.eventSourceMode === "ai" && cfg.apiEnabled && !state.manualPick) {
        const invented = await inventAiEventViaSeparateApi(cfg, state);
        if (invented) {
            state.recentEvents.push(invented.summary);
            trimHistory(cfg, state);
            state.lastEventOrigin = { summary: invented.summary, detail: invented.detail };

            const intensityNote = getIntensityNote(cfg);
            const styleNote = getStyleNote(cfg);
            return `Scene direction: let this shape what happens next in the story: ${invented.detail}. ${intensityNote} ${styleNote} ${PACING_NOTE} ${COMPLETENESS_NOTE} ${ANTI_LEAK_NOTE}${buildGroundingBlock(cfg)}`;
        }
        // Separate call failed for some reason (notify() already fired
        // inside it) — fall back to the old piggyback approach for this one
        // turn rather than silently producing no event at all.
        console.warn("[Story Director] Separate-API event invention failed; falling back to piggyback mode for this turn.");
    }
    return buildInjectionText(cfg, state);
}
...
function buildFollowUpInjectionText(cfg, state, followUp) {
    ...
    return `Scene direction: earlier in this story, this thread was introduced: "${followUp.originSummary}". Check how things actually developed after that point, its real content lives in the preceding chat messages, not here, and continue or escalate that same thread now in this reply — do not introduce a new, unrelated event. ${intensityNote} ${styleNote} ${historyNote} ${PACING_NOTE} ${COMPLETENESS_NOTE} ${SUMMARY_TAG_NOTE} ${ANTI_LEAK_NOTE}${buildGroundingBlock(cfg)}`;
}
...
async function onBeforeGenerate(type, _options, dryRun) {
    ...
    if (shouldFire) {
        ...
        const text = isTimeSkip
            ...
            : await buildInjectionTextAsync(cfg, state);
    ...
    // Zero extra API cost — piggybacks entirely on this same generation call.
    let diaryText = null;
    if (cfg.diaryEnabled) {
        ...
        if (forcedDiary || intervalDue || randomHit) {
            ...
            if (cfg.apiEnabled) {
                // Don't inject anything into THIS generation at all — the
                // diary will be written by its own dedicated API call right
                // after the character's reply lands (see
                // onCharacterMessageReceived). Nothing for the main model to
                // comply with here, so nothing to fail on.
                state.diaryPendingSeparateCall = true;
            } else {
...
function buildPopupHtml(cfg, state) {
    ...
    <div class="sd-popup">
        <div class="sd-accordion" id="sd-accordion">
            ...
            <div class="sd-accordion-item" data-tab-panel="api">
                <button type="button" class="sd-accordion-header" aria-expanded="false">
                    <span class="sd-accordion-title">API แยกสำหรับ Story Director</span>
                    <span class="sd-accordion-caret">▾</span>
                </button>
                <div class="sd-accordion-body" data-acc-body="api" hidden>
            <div class="sd-row">
                <label><input type="checkbox" id="sd-api-enabled" ${cfg.apiEnabled ? "checked" : ""}/> ใช้ API แยกต่างหากสำหรับ Story Director</label>
            </div>
            <div class="sd-help">ค่าเริ่มต้น Story Director จะฝากคำสั่งไปกับข้อความตอบปกติของตัวละครเลย โดยไม่เสีย API เพิ่ม แต่บางโมเดล/บาง API จะไม่ยอมทำตามคำสั่งที่ฝากไปแบบนั้นเลย เปิดโหมดนี้เพื่อให้ Story Director ยิง API อีกครั้งแยกต่างหากจาก API หลักที่ใช้แชทอยู่ (เสีย API เพิ่ม 1 ครั้งต่อครั้งที่มีเหตุการณ์แบบ "AI คิดเอง" หรือไดอารี่เกิดขึ้น) เพื่อความแม่นยำที่สูงขึ้นมาก เพราะไม่ต้องพึ่งให้โมเดลหลักยอมทำตามคำสั่งที่ซ่อนไว้อีกต่อไป</div>

            <div class="sd-row">
                <label>โปรไฟล์ API ที่จะใช้</label>
                <select id="sd-api-profile">
                    <option value="">— เลือกโปรไฟล์ —</option>
                    ${getSavedConnectionProfiles().map((p) => `<option value="${escapeHtml(p.id)}" ${cfg.apiProfileId === p.id ? "selected" : ""}>${escapeHtml(p.name || p.id)}</option>`).join("")}
                </select>
                <button type="button" class="menu_button" id="sd-api-test-btn">ทดสอบการเชื่อมต่อ</button>
            </div>
            <div class="sd-help">รายชื่อนี้ดึงมาจากโปรไฟล์ที่บันทึกไว้ใน "Connection Profiles" ของ SillyTavern (อยู่ในเมนู API หลัก) หากไม่เห็นโปรไฟล์ที่ต้องการ ให้ไปสร้าง/บันทึกโปรไฟล์นั้นในเมนู API ก่อน แล้วเปิดหน้าต่างนี้ใหม่อีกครั้ง ปุ่ม "ทดสอบการเชื่อมต่อ" จะบันทึกโปรไฟล์ที่เลือกไว้ทันทีและลองยิงคำขอสั้น ๆ เพื่อยืนยันว่าเชื่อมต่อได้จริง</div>

            <div class="sd-row">
                <label>Max tokens สำหรับเหตุการณ์ (โหมด "AI คิดเอง")</label>
                <input type="number" id="sd-api-max-event" value="${cfg.apiMaxTokensEvent}" min="50" max="4000"/>
            </div>
            <div class="sd-row">
                <label>Max tokens สำหรับไดอารี่</label>
                <input type="number" id="sd-api-max-diary" value="${cfg.apiMaxTokensDiary}" min="100" max="4000"/>
            </div>
            <div class="sd-help">เมื่อเปิดโหมดนี้ ทั้งการคิดเหตุการณ์แบบ "AI สร้างเอง" (ในแท็บเหตุการณ์) และการเขียนไดอารี่ จะย้ายไปเรียก API แยกนี้แทนการฝากไปกับข้อความหลัก ส่วนเหตุการณ์แบบ Fixed Pool/เลือกเอง และฟีเจอร์ข้ามเวลา ยังคงทำงานแบบฝากไปกับข้อความหลักเหมือนเดิม เพราะไม่ได้ต้องการให้ AI คิดเนื้อหาอะไรขึ้นมาใหม่อยู่แล้ว</div>
                </div>
            </div>
            ...
            <div class="sd-subsection">
                ...
                <div class="sd-help">เมื่อถึงรอบที่กำหนด ตัวละครจะแต่งไดอารี่ขึ้นมาเอง เนื้อหา ป.ล. และอารมณ์ทั้งหมดเขียนโดยโมเดล ฝากมากับคำตอบปกติของตัวละครเลย ไม่เสีย API เพิ่มเลย ส่วนวันที่ สภาพอากาศ และเวลาที่เขียน ระบบจะสุ่มให้ในขั้นตอนสุดท้าย</div>
...
function wireLiveCapture(draft) {
    const fieldHandler = (e) => {
        ...
        switch (t.id) {
            ...
            case "sd-api-enabled": draft.apiEnabled = t.checked; break;
            case "sd-api-profile": draft.apiProfileId = t.value; break;
            case "sd-api-max-event": draft.apiMaxTokensEvent = clampNum(t.value, 300, 50, 4000); break;
            case "sd-api-max-diary": draft.apiMaxTokensDiary = clampNum(t.value, 900, 100, 4000); break;
        }
    };
...
function exportConfigToFile(cfg) {
    const envelope = {
        ...
        config: {
            ...
            apiProfileId: cfg.apiProfileId,
            apiMaxTokensEvent: cfg.apiMaxTokensEvent,
            apiMaxTokensDiary: cfg.apiMaxTokensDiary,
        },
    };
...
function sanitizeImportedConfig(base, raw) {
    ...
    out.apiEnabled = bool(src.apiEnabled, out.apiEnabled);
    out.apiProfileId = str(src.apiProfileId, out.apiProfileId);
    out.apiMaxTokensEvent = num(src.apiMaxTokensEvent, out.apiMaxTokensEvent);
    out.apiMaxTokensDiary = num(src.apiMaxTokensDiary, out.apiMaxTokensDiary);
    ...
    const handler = (e) => {
        ...
        if (target.id === "sd-sound-test-btn") {
            ...
            if (!draft.apiProfileId) {
                notify("กรุณาเลือกโปรไฟล์ API ก่อน", true);
                return;
            }
            // Same trap as the diary/timeskip force buttons above: persist
            // the chosen profile right now so the test call (and any
            // separate-API generation before Save is clicked) uses it.
            if (cfg.apiProfileId !== draft.apiProfileId) {
                cfg.apiProfileId = draft.apiProfileId;
                saveSettingsDebounced();
            }
            notify("กำลังทดสอบการเชื่อมต่อ...");
            requestSeparateGeneration(cfg, "Reply with exactly one word: OK", 10).then((result) => {
                if (result) {
                    notify(`เชื่อมต่อสำเร็จ — คำตอบที่ได้: "${truncateText(result.trim(), 60)}"`);
                } else {
                    notify("การเชื่อมต่อล้มเหลว ดู console (F12) สำหรับรายละเอียด", true);
                }
            });
...
jQuery(async () => {
    ...
    loadConnectionManagerModule(); // fire and forget; requestSeparateGeneration() re-checks/retries on first actual use
