// ============================================================================
// Story Director v3 — a SillyTavern extension that keeps roleplay chats from
// idling into "slice of life forever". A dedicated Director AI call reviews
// the story after every {{char}} reply and decides whether to open a new
// plot thread, push an existing one forward, or resolve one — instead of
// firing disconnected random events. Character Diary entries are long,
// first-person reflections tied to the same threads, not a separate system.
//
...
// ----------------------------------------------------------------------------
// Optional modules used for grounding Director/diary calls in established
// facts (character card, {{user}} persona, active Lorebook/World Info).
// Loaded defensively — if a path doesn't match your SillyTavern version, the
// extension still works, it just skips that one source of grounding.
// ----------------------------------------------------------------------------
...
async function loadOptionalModules() {
    try {
        ...
        if (!power_user_ref) console.warn("[Story Director] power-user.js loaded but no 'power_user' export found; persona grounding will be skipped.");
    } catch (e) {
        console.warn("[Story Director] Could not load power-user.js; persona grounding will be skipped.", e);
    }
    try {
        const wiMod = await import("../../../world-info.js");
        getWorldInfoPromptRef = wiMod.getWorldInfoPrompt || null;
        if (!getWorldInfoPromptRef) console.warn("[Story Director] world-info.js loaded but no 'getWorldInfoPrompt' export found; Lorebook grounding will be skipped.");
    } catch (e) {
        console.warn("[Story Director] Could not load world-info.js; Lorebook grounding will be skipped.", e);
    }
}

// ----------------------------------------------------------------------------
// Separate API (Connection Profile) support. Story Director v3 ALWAYS uses a
// dedicated generation call for the Director decision and for diary entries
// — it never relies on the main roleplay model to comply with an embedded
// instruction, because that silent-failure mode was the single biggest
// source of "I set everything up and nothing happens" reports on v2.
// ----------------------------------------------------------------------------
...
async function loadConnectionManagerModule() {
    ...
    console.warn("[Story Director] Could not find ConnectionManagerRequestService. Requires SillyTavern's built-in Connection Profiles (1.12.6+) enabled.");
}
...
async function requestSeparateGeneration(cfg, promptText, maxTokens) {
    try {
        if (!cfg.apiProfileId) {
            notify('Story Director: ยังไม่ได้เลือกโปรไฟล์ API (ไปที่แท็บ "ทั่วไป" > "API สำหรับ Director")', true);
            return null;
        }
        ...
        const result = await withTimeout(
            ...
            60000,
            "หมดเวลารอคำตอบจาก API (60 วินาที) — ตรวจสอบว่าโปรไฟล์นี้ตั้งค่า API key/URL ถูกต้อง และบริการ/โมเดลนั้นออนไลน์อยู่จริง"
        );
        ...
        notify("Story Director: ได้รับคำตอบจาก API แต่รูปแบบไม่ตรงกับที่คาดไว้ (ดู console)", true);
        return null;
    } catch (e) {
...
const DEFAULT_CHAT_SETTINGS = () => ({
    ...
    // Thread/Director tuning
    maxActiveThreads: 3,
    frequencyMode: "random",      // 'interval' | 'random' — how often the Director gets asked to look at the story at all
    intervalN: 6,
    randomChance: 20,             // percent, checked after every {{char}} message
    eventIntensity: "medium",     // 'mild' | 'medium' | 'disruptive'
    presentation: "subtle",       // 'subtle' | 'narrator' | 'both'
    dormantMemory: 6,             // how many resolved threads the Director is shown, to avoid repeats
    ...
    // Diary
    diaryEnabled: true,
    diaryAdvanceChance: 50,       // % chance a plain "advance" beat (not open/resolve) also gets a diary entry
    diaryStickersEnabled: true,
    diaryBannerEnabled: true,

    // In-story calendar (see storyCalendar in chat state)
    calendarStartDate: "",        // ISO date; blank = the real date this thread's calendar was first touched

    // Separate API — required, not optional, in v3
    apiProfileId: "",
    apiMaxTokensDirector: 500,
    apiMaxTokensDiary: 1800,

    // Debug — preview the exact Director prompt without spending a call
...
function getChatSettings() {
    ...
    // Backfill for chats created before a field existed.
    const fresh = DEFAULT_CHAT_SETTINGS();
    for (const k of Object.keys(fresh)) {
        if (cfg[k] === undefined) cfg[k] = fresh[k];
    }
...
// ----------------------------------------------------------------------------
// Per-chat state: active/dormant plot threads, diary log, and the in-story
// calendar. Stored on chatMetadata so it saves/loads with the chat file.
// ----------------------------------------------------------------------------
...
function getChatState() {
    ...
    if (!context.chatMetadata[MODULE_NAME]) {
        context.chatMetadata[MODULE_NAME] = {
            activeThreads: [],       // { id, title, status, originSummary, beats:[], emotionalCharge, diaryEntryIds:[] }
            dormantThreads: [],      // resolved threads, most recent last, capped
            diaryLog: [],            // { id, dateIso, weatherText, temperature, mood, moodColor, body, threadId, read }
            eventLog: [],            // { summary, action, time } — for the History tab
            totalFired: 0,
            lastProcessedMesId: -1,  // guards against swipes/dup MESSAGE_RECEIVED firing twice for the same slot
            messagesSinceCheck: 0,
            forceNextCheck: false,   // set by the "เร่งการตัดสินใจ" button; consumed on the next {{char}} message
            storyCalendar: null,     // lazily created — see ensureCalendar()
        };
    }
...
// ----------------------------------------------------------------------------
// Grounding — character card + persona + active Lorebook/World Info, folded
// into both the Director prompt and the diary prompt so neither invents
// facts that contradict the established story.
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// In-story calendar. No dedicated Time Skip feature in v3 — the Director
// estimates elapsed time as part of its normal decision instead, so time
// keeps moving without a separate competing subsystem.
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// Director — the core decision engine. Runs as its own generation call after
// every {{char}} message, reads the active/dormant threads, and decides
// whether to open, advance, resolve, or do nothing.
// ----------------------------------------------------------------------------
...
async function runDirectorDecision(cfg, state) {
    ...
    if (!parsed || !parsed.action) {
        ...
        notify("Story Director: การตอบกลับของ Director ไม่ใช่ JSON ที่รูปแบบถูกต้อง (ดู console)", true);
        return null;
    }
    return parsed;
}
...
function applyDirectorDecision(cfg, state, decision) {
    ...
    if (decision.action === "open_new") {
        if (state.activeThreads.length >= Math.max(1, cfg.maxActiveThreads)) {
            // Director tried to open past the cap — fall back to treating it
            // as advancing the least-recently-touched active thread instead
            // of silently dropping the beat.
...
// ----------------------------------------------------------------------------
// Character Diary — long, first-person, tied to whichever thread beat just
// happened. Written by its own dedicated API call, never piggybacked.
// ----------------------------------------------------------------------------
...
async function writeDiaryForThread(cfg, state, threadResult) {
    ...
    if (!parsed || !parsed.body) {
        ...
        notify("Story Director: การตอบกลับของไดอารี่ไม่ใช่ JSON ที่รูปแบบถูกต้อง (ดู console)", true);
        return null;
    }
...
function playEventSound(styleKey) {
    try {
        ...
        if (playPromise && typeof playPromise.catch === "function") {
            playPromise.catch((e) => console.warn("[Story Director] Sound playback blocked or failed:", e));
        }
    } catch (e) {
...
// ----------------------------------------------------------------------------
// Main engine loop — hooked to MESSAGE_RECEIVED. Runs after every {{char}}
// reply: consumes whatever injection guided THIS reply, then (if due) asks
// the Director for the NEXT one. No GENERATION_STARTED hook is needed at all
// — everything happens in the background between turns, so it never adds
// latency to the generation the user is actually waiting on.
// ----------------------------------------------------------------------------
...
async function onCharacterMessageReceived(mesId) {
    try {
        ...
        // Whatever was injected for THIS reply has now been used — clear it
        // so it doesn't silently carry over to later turns.
        clearInjection();
...
function buildPopupHtml(cfg, state) {
    ...
    <div class="sd-popup">
        <div class="sd-accordion" id="sd-accordion">
            ...
            <div class="sd-accordion-item" data-tab-panel="general">
                <button type="button" class="sd-accordion-header" aria-expanded="true">
                    <span class="sd-accordion-title">⚙️ ทั่วไป</span>
                ...
                <div class="sd-accordion-body" data-acc-body="general">
                    ...
                    <div class="sd-help">การตั้งค่าที่บันทึกไว้จะยังอยู่ครบแม้ปิดใช้งานชั่วคราว</div>

                    <div class="sd-subsection">
                        <h4>API สำหรับ Director (จำเป็น)</h4>
                        <div class="sd-help">Director และไดอารี่ใช้ API แยกเสมอ — ไม่พึ่งพาให้โมเดลหลักของแชท "ทำตาม" คำสั่งแฝงเหมือนเวอร์ชันก่อน เพื่อไม่ให้เกิดปัญหาตั้งค่าไว้แล้วไม่มีอะไรเกิดขึ้น</div>
                        <div class="sd-row">
                    ...
                    <div class="sd-help">มีผลทันทีไม่ต้องกดบันทึก — Director จะตัดสินใจทันทีหลังข้อความถัดไปของตัวละคร แทนที่จะรอความถี่ปกติ</div>

                    <div class="sd-row"><label><input type="checkbox" id="sd-sound-enabled" ${cfg.soundEnabled ? "checked" : ""}/> เล่นเสียงสั้นๆ เมื่อ Director ตัดสินใจทำอะไรบางอย่าง</label></div>
                    <div class="sd-row">
                    ...
                    <div class="sd-subsection">
                        ...
                        <div class="sd-help" id="sd-io-status">ส่งออก/นำเข้าเฉพาะการตั้งค่า ไม่รวมเธรด/ไดอารี่/ประวัติของแชทนี้ นำเข้าแล้วต้องกด <b>บันทึก</b></div>
                    </div>
                </div>
            </div>

            <div class="sd-accordion-item" data-tab-panel="threads">
                <button type="button" class="sd-accordion-header" aria-expanded="false">
                    <span class="sd-accordion-title">🧵 เธรด/เหตุการณ์</span>
                    <span class="sd-accordion-caret">▾</span>
                </button>
                <div class="sd-accordion-body" data-acc-body="threads" hidden>
                    <div class="sd-row"><label>ธีมที่อนุญาต</label></div>
                    <div class="sd-tags">${renderTagPicker(cfg)}</div>
                    <div class="sd-help">ไม่เลือกเลย = ปล่อยอิสระตามโทนเรื่องปัจจุบัน</div>
                    ...
                    <div class="sd-row"><label><input type="checkbox" id="sd-preview-enabled" ${cfg.promptPreviewEnabled ? "checked" : ""}/> เปิด Prompt Preview (ดูพรอมต์ที่จะส่งให้ Director โดยไม่เสีย API call)</label></div>
            ...
            <div class="sd-accordion-item" data-tab-panel="diary">
                ...
                <div class="sd-accordion-body" data-acc-body="diary" hidden>
                    ...
                    <div class="sd-help">เธรดที่เพิ่งเปิดใหม่หรือเพิ่งปิด (จุดพีค) จะมีไดอารี่เสมอ — ค่านี้คุมเฉพาะตอนเธรดแค่คืบหน้าทีละนิด</div>
                    ...
                    <div class="sd-help">เว้นว่าง = ใช้วันที่จริงตอนเธรดแรกถูกเปิด</div>
            ...
            <div class="sd-accordion-item" data-tab-panel="history">
                <button type="button" class="sd-accordion-header" aria-expanded="false">
                    <span class="sd-accordion-title">📊 สถิติ/ประวัติ</span>
                    <span class="sd-accordion-caret">▾</span>
                </button>
                <div class="sd-accordion-body" data-acc-body="history" hidden>
        ...
        <div class="sd-credit">Story Director v3 — Purryverse</div>
    </div>`;
}
    ...
    const handler = (e) => {
        ...
        if (target.id === "sd-force-btn") {
            ...
            requestSeparateGeneration(cfg, "Reply with exactly one word: OK", 10).then((result) => {
                ...
                else notify("การเชื่อมต่อล้มเหลว ดู console (F12) สำหรับรายละเอียด", true);
            });
...
function importConfigFromFile(draft, raw) {
    ...
    for (const key of Object.keys(fresh)) {
        if (key === "apiProfileId" || key === "promptPreviewEnabled") continue; // never import profile id or debug toggle
...
function addWandMenuButton() {
    ...
    if (!menu) {
        ...
        if (addWandMenuButtonAttempts > 40) {
            console.error("[Story Director] #extensionsMenu never appeared after 20s — giving up.");
            return;
        }
    ...
    item.tabIndex = 0;
    item.innerHTML = `<i class="fa-fw fa-solid fa-wand-magic-sparkles"></i><span>Story Director</span>`;
...
jQuery(async () => {
    ...
    loadConnectionManagerModule(); // fire and forget
    refreshWorldInfoCache();       // fire and forget
    ...
    if (event_types.MESSAGE_RECEIVED) {
        ...
        console.warn("[Story Director] MESSAGE_RECEIVED not found; the extension cannot function.");
    }

    console.log("[Story Director] v3 loaded.");
});
