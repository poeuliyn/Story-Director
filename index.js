...
async function onCharacterMessageReceived(mesId) {
    try {
        ...
        if (recapDue) {
            ...
            notify(`Story Director: แทรกสรุป "ความเดิมตอนที่แล้ว" ให้แล้ว (ห่างหายไปนาน)`);
        }
    ...
    <div class="sd-popup">
        ...
        <div class="sd-tab-panel" data-tab-panel="general">
            ...
            <div class="sd-subsection" style="margin-top:8px;">
                ...
                <div class="sd-help">อัปโหลดไฟล์เสียงสั้นๆ ของตัวเอง (แนะนำไม่เกิน 3-4 วินาที) ไฟล์เล็กกว่า 300KB เพิ่มได้สูงสุด ${MAX_CUSTOM_SOUNDS} เสียง เพิ่มแล้วจะไปโผล่ในช่อง "เลือกเสียง" ด้านบนให้เลือกได้เลย</div>
        ...
        <div class="sd-tab-panel sd-hidden" data-tab-panel="settings">
            ...
            <div class="sd-subsection">
                ...
                <div class="sd-help">กำหนดโทนที่ Director ใช้ตัดสินใจแต่ละครั้ง เลือก "สุ่มทุกครั้ง" เพื่อให้บุคลิกสลับไปมาเองในแต่ละการตัดสินใจ ช่วยให้เรื่องไม่นิ่งไปทางเดียวตลอด</div>
                <div class="sd-row">
            ...
            <div class="sd-subsection" style="margin-top:8px;">
                <h4 style="margin-top:0;"><label><input type="checkbox" id="sd-recap-enabled" ${cfg.recapEnabled ? "checked" : ""}/> "ความเดิมตอนที่แล้ว" อัตโนมัติ</label></h4>
                <div class="sd-help">เมื่อกลับมาเล่นต่อหลังห่างหายไปนาน (นับเวลาจริง ไม่ใช่เวลาในเรื่อง) ระบบจะแทรกให้ตัวละครพูดถึงหรือให้ narrator สรุปสั้นๆ ว่าเธรดไหนค้างอะไรอยู่ ก่อนดำเนินฉากถัดไป ทำครั้งเดียวตอนกลับมาแต่ละรอบ ไม่ต้องเรียก API เพิ่ม</div>
            ...
            <div class="sd-subsection" style="margin-top:8px;">
                <h4 style="margin-top:0;"><label><input type="checkbox" id="sd-npc-events-enabled" ${cfg.npcEventsEnabled ? "checked" : ""}/> เหตุการณ์ที่มี NPC เกี่ยวข้อง</label></h4>
                <div class="sd-help">เมื่อเปิดใช้งาน Director จะพิจารณาดึงตัวละครสมทบ (NPC) เข้ามาเกี่ยวข้องกับเหตุการณ์ต่างๆ เพื่อให้โลกในเรื่องรู้สึกมีชีวิตขึ้น ไม่จำเป็นต้องมีทุกครั้ง เพิ่มชื่อ NPC ที่อยากให้ใช้ไว้ในทำเนียบด้านล่างได้ (ไม่บังคับ — Director แต่ง NPC ใหม่เองได้เมื่อเข้ากับบริบท) ดูแผนที่ความสัมพันธ์ระหว่าง NPC ด้วยกันได้ในแท็บ "เธรด"</div>
            ...
            <div class="sd-subsection" style="margin-top:8px;">
                <h4 style="margin-top:0;"><label><input type="checkbox" id="sd-secrets-enabled" ${cfg.secretsEnabled ? "checked" : ""}/> ความลับของตัวละคร (Secrets)</label></h4>
                <div class="sd-help">ตั้งความลับของตัวละคร/NPC ไว้ล่วงหน้า Director จะค่อยๆ ปล่อยเบาะแสผ่านไดอารี่/เหตุการณ์ทีละนิด แล้วเลือกจังหวะเฉลยเองเมื่อสะสมเบาะแสมากพอ เขียนเนื้อหาความลับเองก็ได้ หรือเว้นช่องเนื้อหาว่างไว้เพื่อให้ Director แต่งขึ้นเองตอนเริ่มใช้ ดูความลับที่กำลังดำเนินอยู่ได้ในแท็บ "เธรด"</div>
            ...
            <div class="sd-subsection" style="margin-top:8px;">
                <h4 style="margin-top:0;"><label><input type="checkbox" id="sd-callback-enabled" ${cfg.callbackEnabled ? "checked" : ""}/> Callback อดีต</label></h4>
                <div class="sd-help">ให้ Director หยิบรายละเอียดเล็กๆ จากไดอารี่/เธรดที่ปิดไปนานแล้ว (ที่ผู้เล่นอาจลืมไปแล้ว) กลับมาโผล่ใหม่แบบไม่ทันตั้งตัวเป็นครั้งคราว ทำให้โลกในเรื่องรู้สึกว่าจดจำได้จริง</div>
                <div class="sd-row"><label>นับเป็น "อดีต" เมื่อเก่ากว่า (ข้อความ)</label><input type="number" id="sd-callback-min-age" min="5" max="500" value="${cfg.callbackMinAge}"/></div>
            </div>

            <div class="sd-subsection" style="margin-top:8px;">
                <h4 style="margin-top:0;"><label><input type="checkbox" id="sd-story-arcs-enabled" ${cfg.storyArcsEnabled ? "checked" : ""}/> โครงเรื่องระยะยาวแบบซีซั่น (Story Arcs)</label></h4>
                <div class="sd-help">แทนที่จะมองแค่เธรดสั้นๆ ให้ Director วางแผน "อาร์ค" ใหญ่ล่วงหน้า มีชื่ออาร์คและจุดไคลแมกซ์ที่วางแผนไว้ ให้ความรู้สึกเหมือนซีรีส์ที่มีซีซั่นแทนที่จะเป็นเหตุการณ์สุ่มเรียงกัน ดูความคืบหน้าของอาร์คปัจจุบันได้ในแท็บ "เธรด"</div>
