...
let connectionManagerLoadAttempted = false;

async function loadConnectionManagerModule() {
    if (ConnectionManagerRequestServiceRef || connectionManagerLoadAttempted) return;
    connectionManagerLoadAttempted = true;
    // Different SillyTavern versions have exposed this from different files;
    // try the known candidates in order and keep whichever one resolves.
    ...
    console.warn(
        "[Story Director] Could not find ConnectionManagerRequestService under any known import path. " +
        "The separate-API feature requires SillyTavern's built-in 'Connection Profiles' extension " +
        "(1.12.6+) to be enabled. If it IS enabled and this still fails, your SillyTavern version " +
        "likely exports this from a different file — please report the version so the path can be fixed."
    );
}
...
async function requestSeparateGeneration(cfg, promptText, maxTokens) {
    try {
        ...
        if (!ConnectionManagerRequestServiceRef) {
            notify("Story Director: ไม่พบระบบ Connection Profile ใน SillyTavern เวอร์ชันนี้ ดู console (F12) สำหรับรายละเอียด", true);
            return null;
        }
